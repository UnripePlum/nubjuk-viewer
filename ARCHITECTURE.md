...
| `motion_started` | **현재 cid 비교** (stale 거부) → motion.play(intent, expectedMs, cid) |
| `motion_completed` | 현재 cid 비교 → store에 표시 (motion에는 안 보냄, motion이 자체 timer로 결정) |
| `motion_failed` | 현재 cid 비교 → motion.stop() + store에 reason 표시 |
| `error` | store.protocolErrors / 알림 토스트 (code별로 UI 다르게) |
| `heartbeat` | last received ts_ms 갱신 (timeout 감지용) |
| schema 위반 | onProtocolError → store.protocolErrors 카운터 증가 + 콘솔 경고 |
...
## Motion 라이프사이클 (correlation_id 기반)

```mermaid
stateDiagram-v2
    [*] --> idle
    idle --> playing: play(intent, ms, cid_A)
    playing --> idle: setTimeout(ms) → completed event
    playing --> idle: stop() → cancelled event (stop_called)
    playing --> playing: play(intent, ms, cid_B) → 현재 cancelled (superseded) + 새로 시작
```

**stale 메시지 거부**:
- ESP에서 `motion_completed{cid: cid_old}` 가 늦게 도착 → IntentDispatcher가 `motion.getCurrentCorrelationId() !== cid_old` 확인 후 motion에는 전달 X (store에만 로그)
